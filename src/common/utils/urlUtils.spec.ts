...
describe('getHostname', () => {
  test('should extract hostname from valid URLs', () => {
    ...
    expect(UrlUtils.getHostname('http://sub.example.co.com')).toBe(
      'sub.example.co.com'
    );
    expect(UrlUtils.getHostname('https://www.google.com/search?q=test')).toBe(
      'www.google.com'
    );
  });
  ...
  test('should handle URLs with authentication info', () => {
    expect(UrlUtils.getHostname('https://user:pass@example.com')).toBe(
      'example.com'
    );
    expect(UrlUtils.getHostname('http://admin:1234@sub.example.com')).toBe(
      'sub.example.com'
    );
  });

  test('should handle URLs with unusual but valid formats', () => {
    expect(UrlUtils.getHostname('http://[2001:db8::1]:8080')).toBe(
      '[2001:db8::1]'
    );
  });
});
