...
export class Options {
  ...
  private getElements() {
    return {
      ...
      websiteTemplate:
        DomUtils.getElement<HTMLTemplateElement>('website-template'),
  ...
  private createWebsiteElement(domain: string, config: DomainConfig): void {
    const websiteItem = this.elements.websiteTemplate.content.cloneNode(
      true
    ) as DocumentFragment;
  ...
  private setupWebsiteHeader(container: HTMLElement, domain: string): void {
    const domainElement = container.querySelector(
      '.website-domain'
    ) as HTMLElement;
  ...
  private toggleConfigSection(
    container: HTMLElement,
    toggleImage?: HTMLImageElement
  ): void {
    const configSection = container.querySelector(
      '.website-config'
    ) as HTMLElement;
  ...
  private setupDeleteButton(container: HTMLElement, domain: string): void {
    ...
    deleteButton.addEventListener('click', async (e) => {
      e.stopPropagation();
      if (
        confirm(`Are you sure you want to remove TabShield from ${domain}?`)
      ) {
  ...
  private setupColorPicker(
    ...
    const colorOptions = container.querySelectorAll(
      '.color-option'
    ...
    colorOptions.forEach((option) => {
      ...
      option.addEventListener('click', async () => {
        ...
        const color =
          ...
          ConfigService.DEFAULT_CONFIG.labelColor;
  ...
  private exportConfiguration(): void {
    try {
      ...
      this.downloadFile(
        url,
        `tabshield-config-${new Date().toISOString().split('T')[0]}.json`
      );
  ...
  private validateFile(file: File) {
    if (file.size > Options.FILE_MAX_SIZE) {
      throw new Error(
        `File is too large. Max size is ${Options.FILE_MAX_SIZE / 1000000}MB`
      );
    }
  }
  ...
  private async mergeOrReplaceConfig(
    importedConfig: Record<string, DomainConfig>
    ...
    const merge = confirm(
      'Merge with existing configuration? OK to merge, Cancel to replace all configuration'
    );
  ...
  private validateConfig(config: any) {
    ...
    for (const domain in config) {
      ...
      if (
        typeof domainConfig.displayLabel !== 'boolean' ||
      ...
      if (
        domainConfig.label !== undefined &&
        typeof domainConfig.label !== 'string'
      ) {
        throw new Error('Invalid configuration');
      }
    }
